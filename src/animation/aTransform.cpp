...
ATransform ATransform::Inverse() const
{
	...
	ATransform i;
	i.m_rotation = this->m_rotation.Transpose();
	i.m_translation = -this->m_translation;
...
vec3 ATransform::RotTrans(const vec3& vecToTransform) const
{
	...
	vec3 r;
	r = Translate(Rotate(vecToTransform));
...
vec3 ATransform::Rotate(const vec3& vecToTransform) const
{
	...
	vec3 r;
	r = this->m_rotation * vecToTransform;
...
vec3 ATransform::Translate(const vec3& vecToTransform) const
{
	...
	vec3 r;
	r = this->m_translation + vecToTransform;
...
ATransform operator * (const ATransform& H1, const ATransform& H2)
{
	...
	ATransform r;
	r.m_rotation = H1.m_rotation * H2.m_rotation;
	r.m_translation = H1.m_translation + H1.m_rotation * H2.m_translation;
